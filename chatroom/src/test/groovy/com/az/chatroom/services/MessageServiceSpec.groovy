...
class MessageServiceSpec extends Specification {
    ...
    def "should reject message from missing user"() {
        ...
        exception.message == "Message author not found."
        0 * _
    }
    ...
    def "should delete existing message"() {
        ...
        then:
        1 * messageRepository.delete(TEST_MESSAGE_ID) >> true
        noExceptionThrown()
        0 * _
    }

    def "should reject deleting missing message"() {
        when:
        messageService.deleteMessage(TEST_MESSAGE_ID)

        then:
        1 * messageRepository.delete(TEST_MESSAGE_ID) >> false
        def exception = thrown(ResourceNotFoundException)
