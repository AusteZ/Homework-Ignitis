...
class MessageRepositorySpec extends Specification {
    ...
    def "should delete existing message"() {
        given:
        insertUser()
        insertMessage(TEST_MESSAGE_ID, TEST_USER_ID, TEST_MESSAGE_CONTENT, TEST_DATE_TIME)

        when:
        def deleted = messageRepository.delete(TEST_MESSAGE_ID)

        then:
        deleted
        !dsl.fetchExists(CHAT_MESSAGE, CHAT_MESSAGE.ID.eq(TEST_MESSAGE_ID))
    }

    def "should return false when deleting non-existing message"() {
        expect:
        !messageRepository.delete(TEST_MESSAGE_ID)
    }
